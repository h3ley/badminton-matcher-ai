        // --- DOM Elements ---
        const addPlayersBtn = document.getElementById('add-players-btn');
        const playerInputList = document.getElementById('player-input-list');
        const playerListContainer = document.getElementById('player-list-container');
        const playerTotalCount = document.getElementById('player-total-count');
        const courtCountInput = document.getElementById('court-count');
        const generateRoundBtn = document.getElementById('generate-round-btn');
        const resetBtn = document.getElementById('reset-btn');
        const clearAllBtn = document.getElementById('clear-all-btn');
        const resultsSection = document.getElementById('results-section');
        const roundCounter = document.getElementById('round-counter');
        const reshuffleBtn = document.getElementById('reshuffle-btn');
        const currentRoundContainer = document.getElementById('current-round-container');
        const restingPlayersContainer = document.getElementById('resting-players-container');
        const restingList = document.getElementById('resting-list');
        const messageArea = document.getElementById('message-area');
        const historySection = document.getElementById('history-section');
        const historyContainer = document.getElementById('history-container');
        const toggleStatsBtn = document.getElementById('toggle-stats-btn');
        const statsContainer = document.getElementById('stats-container');
        const statsList = document.getElementById('stats-list');
        
        // Modals
        const playerModal = document.getElementById('player-modal');
        const modalPlayerList = document.getElementById('modal-player-list');
        const modalCloseBtn = document.getElementById('modal-close-btn');
        const confirmModal = document.getElementById('confirm-modal');
        const confirmTitle = document.getElementById('confirm-title');
        const confirmMessage = document.getElementById('confirm-message');
        const confirmYesBtn = document.getElementById('confirm-yes-btn');
        const confirmNoBtn = document.getElementById('confirm-no-btn');

        // --- Application State ---
        const STORAGE_KEY = 'badmintonMatcherState';
        let players = []; // Array of { id, name, gamesPlayed, level, consecutiveRests }
        let currentMatch = { round: 0, courts: [], resting: [] };
        let history = []; // Array of past matches
        let round = 0;
        let editingContext = null; // { historyIndex, courtIndex, teamIndex, playerIndex }
        let pendingAction = null; // For confirmation modal
        const skillLevels = ['C', 'B', 'A'];
        const skillScores = { 'C': 1, 'B': 2, 'A': 3 };
        let partnershipHistory = {}; // Key: "p1Id-p2Id", Value: count

        // --- FORWARD DECLARATIONS ---
        let renderAll, renderPlayerList, renderMatches, renderHistory, createPlayerHTML, renderPartnershipStats;

        // --- State Management ---
        function saveState() {
            const state = { players, currentMatch, history, round, courtCount: parseInt(courtCountInput.value, 10), partnershipHistory };
            localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
        }

        function loadState() {
            const savedState = localStorage.getItem(STORAGE_KEY);
            if (savedState) {
                const state = JSON.parse(savedState);
                players = state.players || [];
                players.forEach(p => { 
                    if (!p.level) p.level = 'C'; 
                    if (!p.consecutiveRests) p.consecutiveRests = 0;
                });
                currentMatch = state.currentMatch || { round: 0, courts: [], resting: [] };
                history = state.history || [];
                round = state.round || 0;
                courtCountInput.value = state.courtCount || 2;
                partnershipHistory = state.partnershipHistory || {};

                if (round > 0) resultsSection.style.display = 'block';
                if (history.length > 0) historySection.style.display = 'block';
                renderAll();
            }
        }

        // --- Player Management ---
        function handleAddPlayers() {
            const names = playerInputList.value.split('\n').map(name => name.replace(/^\d+\.\s*/, '').trim()).filter(Boolean);
            names.forEach(name => {
                if (!players.some(p => p.name === name)) {
                    players.push({ id: Date.now() + Math.random(), name, gamesPlayed: 0, level: 'C', consecutiveRests: 0 });
                }
            });
            playerInputList.value = '';
            renderAll();
            saveState();
        }

        function removePlayer(playerId) {
            players = players.filter(p => p.id !== playerId);
            renderAll();
            saveState();
        }

        function changePlayerLevel(playerId) {
            const player = players.find(p => p.id === playerId);
            if (player) {
                player.level = skillLevels[(skillLevels.indexOf(player.level) + 1) % skillLevels.length];
                renderPlayerList();
                saveState();
            }
        }

        // --- Core Matching Logic ---
        function shuffleArray(array) {
            for (let i = array.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                [array[i], array[j]] = [array[j], array[i]];
            }
        }

        function createBalancedMatches(playerPool) {
            let courts = [];
            let pool = [...playerPool];
            
            const cPlayers = pool.filter(p => p.level === 'C');
            while (cPlayers.length >= 4) {
                const courtPlayers = cPlayers.splice(0, 4);
                shuffleArray(courtPlayers);
                courts.push({ team1: [courtPlayers[0], courtPlayers[1]], team2: [courtPlayers[2], courtPlayers[3]] });
                pool = pool.filter(p => !courtPlayers.some(cp => cp.id === p.id));
            }

            pool.sort((a, b) => skillScores[b.level] - skillScores[a.level]);
            const teams = [];
            while(pool.length >= 2) {
                const player1 = pool.shift();
                const player2 = pool.pop();
                const key = [player1.id, player2.id].sort().join('-');
                teams.push({
                    players: [player1, player2],
                    score: skillScores[player1.level] + (player2 ? skillScores[player2.level] : 0),
                    penalty: (partnershipHistory[key] || 0) * 100
                });
            }

            const possibleMatches = [];
            for (let i = 0; i < teams.length; i++) {
                for (let j = i + 1; j < teams.length; j++) {
                    possibleMatches.push({
                        teams: [teams[i], teams[j]],
                        score: Math.abs(teams[i].score - teams[j].score) + teams[i].penalty + teams[j].penalty,
                        indices: [i, j]
                    });
                }
            }
            possibleMatches.sort((a, b) => a.score - b.score);

            const usedTeamIndices = new Set();
            for (const match of possibleMatches) {
                if (!usedTeamIndices.has(match.indices[0]) && !usedTeamIndices.has(match.indices[1])) {
                    courts.push({ team1: match.teams[0].players, team2: match.teams[1].players });
                    usedTeamIndices.add(match.indices[0]);
                    usedTeamIndices.add(match.indices[1]);
                }
            }
            
            shuffleArray(courts);
            return courts;
        }

        // --- Round Management ---
        function generateNewRound() {
            const numCourts = parseInt(courtCountInput.value, 10);
            const playersPerRound = numCourts * 4;

            if (players.length < playersPerRound) {
                messageArea.textContent = `ผู้เล่นไม่พอสำหรับ ${numCourts} คอร์ด (ต้องการ ${playersPerRound} คน, มี ${players.length} คน)`;
                return;
            }
            messageArea.textContent = '';

            if (round > 0) {
                history.push(JSON.parse(JSON.stringify(currentMatch)));
            }

            round++;
            resultsSection.style.display = 'block';
            if (history.length > 0) historySection.style.display = 'block';

            let availablePlayers = [...players];
            shuffleArray(availablePlayers);
            availablePlayers.sort((a,b) => a.gamesPlayed - b.gamesPlayed);
            let playingPool = availablePlayers.slice(0, playersPerRound);
            
            const courts = createBalancedMatches(playingPool);
            
            const playingNowIds = new Set(courts.flatMap(c => [...c.team1, ...c.team2]).map(p => p.id));

            players.forEach(p => {
                if (playingNowIds.has(p.id)) {
                    p.gamesPlayed++;
                    p.consecutiveRests = 0;
                } else {
                    p.consecutiveRests++;
                }
            });

            courts.forEach(court => {
                const t1k = [court.team1[0].id, court.team1[1].id].sort().join('-');
                const t2k = [court.team2[0].id, court.team2[1].id].sort().join('-');
                partnershipHistory[t1k] = (partnershipHistory[t1k] || 0) + 1;
                partnershipHistory[t2k] = (partnershipHistory[t2k] || 0) + 1;
            });

            currentMatch = {
                round,
                courts: courts.map((c, i) => ({ ...c, courtNum: i + 1 })),
                resting: players.filter(p => !playingNowIds.has(p.id))
            };
            
            renderAll();
            saveState();
        }

        function reshuffleCurrentRound() {
            if (round === 0) return;

            const playersInCurrentMatch = currentMatch.courts.flatMap(c => [...c.team1, ...c.team2]).filter(Boolean);
            const restingPlayersInCurrentMatch = currentMatch.resting;

            playersInCurrentMatch.forEach(p_in_match => {
                const player = players.find(p => p.id === p_in_match.id);
                if (player) {
                    player.gamesPlayed--;
                    player.consecutiveRests = 1; 
                }
            });
            restingPlayersInCurrentMatch.forEach(p_in_rest => {
                const player = players.find(p => p.id === p_in_rest.id);
                if (player && player.consecutiveRests > 0) {
                    player.consecutiveRests--;
                }
            });

            currentMatch.courts.forEach(court => {
                if (court.team1[0] && court.team1[1]) {
                    const t1k = [court.team1[0].id, court.team1[1].id].sort().join('-');
                    if (partnershipHistory[t1k]) partnershipHistory[t1k]--;
                }
                if (court.team2[0] && court.team2[1]) {
                    const t2k = [court.team2[0].id, court.team2[1].id].sort().join('-');
                    if (partnershipHistory[t2k]) partnershipHistory[t2k]--;
                }
            });

            round--; 
            generateNewRound();
        }

        function reshuffleSingleCourt(courtIndex) {
            if (!currentMatch || !currentMatch.courts[courtIndex]) return;

            const court = currentMatch.courts[courtIndex];
            const originalPlayersInCourt = [...court.team1, ...court.team2].filter(Boolean);
            
            shuffleArray(originalPlayersInCourt);
            
            const newTeam1 = [originalPlayersInCourt[0], originalPlayersInCourt[1]];
            const newTeam2 = [originalPlayersInCourt[2], originalPlayersInCourt[3]];

            currentMatch.courts[courtIndex].team1 = newTeam1;
            currentMatch.courts[courtIndex].team2 = newTeam2;

            renderMatches();
            saveState();
        }

        // --- Rendering Functions (Defined here) ---
        renderAll = function() {
            renderPlayerList();
            renderMatches();
            renderHistory();
            if (!statsContainer.classList.contains('hidden')) {
                renderPartnershipStats();
            }
        }

        renderMatches = function() {
            roundCounter.textContent = `รอบปัจจุบัน (รอบที่ ${round})`;
            currentRoundContainer.innerHTML = '';
            restingList.innerHTML = '';
            
            if (round === 0) {
                resultsSection.style.display = 'none';
                return;
            }
            
            restingPlayersContainer.className = 'mt-6 bg-sky-50 p-4 rounded-2xl shadow-sm transition-colors duration-300 border border-sky-200';
            restingPlayersContainer.querySelector('h3').className = 'text-lg font-semibold text-sky-800 mb-3';

            const currentRoundCard = document.createElement('div');
            currentRoundCard.className = 'bg-sky-50 p-4 rounded-2xl shadow-sm fade-in border border-sky-200';

            let courtsHTML = currentMatch.courts.map((court, courtIndex) => `
                <div class="flex items-center justify-around text-center text-sm py-2 ${currentMatch.courts.length > 1 && courtIndex < currentMatch.courts.length - 1 ? 'border-b border-sky-200' : ''}">
                    <div class="font-semibold text-sky-700 w-20 flex items-center gap-2">
                        <span>คอร์ด ${court.courtNum}</span>
                        <button class="reshuffle-court-btn text-sky-500 hover:text-sky-700" data-court-index="${courtIndex}" title="สุ่มคอร์ดนี้ใหม่">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M3 12a9 9 0 0 0 9 9 9.75 9.75 0 0 0 6.74-2.74L21 16"/><path d="M21 21v-5h-5"/></svg>
                        </button>
                    </div>
                    <div class="team flex gap-1.5 w-32 justify-center">
                        ${createPlayerHTML(court.team1[0], null, courtIndex, 0, 0)}
                        ${createPlayerHTML(court.team1[1], null, courtIndex, 0, 1)}
                    </div>
                    <span class="text-slate-400 font-bold text-base">VS</span>
                    <div class="team flex gap-1.5 w-32 justify-center">
                        ${createPlayerHTML(court.team2[0], null, courtIndex, 1, 0)}
                        ${createPlayerHTML(court.team2[1], null, courtIndex, 1, 1)}
                    </div>
                </div>
            `).join('');

            currentRoundCard.innerHTML = courtsHTML;
            currentRoundContainer.appendChild(currentRoundCard);


            if (currentMatch.resting.length > 0) {
                currentMatch.resting.forEach(player => {
                    const fullPlayer = players.find(p => p.id === player.id);
                    const restCount = fullPlayer ? fullPlayer.consecutiveRests : 0;
                    const restingTag = document.createElement('span');
                    restingTag.className = 'bg-sky-100 text-sky-800 text-xs font-medium px-2.5 py-1 rounded-full flex items-center gap-1.5';
                    restingTag.innerHTML = `
                        <span>${player.name}</span>
                        ${restCount > 1 ? `<span class="bg-amber-200 text-amber-800 text-xs font-bold w-4 h-4 flex items-center justify-center rounded-full">${restCount}</span>` : ''}
                    `;
                    restingList.appendChild(restingTag);
                });
            } else {
                restingList.innerHTML = `<p class="text-slate-400 text-sm">ไม่มีผู้เล่นพัก</p>`;
            }
        }
        
        renderHistory = function() {
            historyContainer.innerHTML = '';
            [...history].reverse().forEach((match, reverseIndex) => {
                const historyIndex = history.length - 1 - reverseIndex;
                const historyCard = document.createElement('div');
                historyCard.className = 'history-card bg-white p-4 rounded-2xl shadow-sm fade-in border';
                
                let courtsHTML = match.courts.map((court, courtIndex) => `
                    <div class="flex items-center justify-around text-center text-xs py-1.5 ${match.courts.length > 1 && courtIndex < match.courts.length - 1 ? 'border-b' : ''}">
                        <div class="font-semibold text-slate-500 w-14">คอร์ด ${court.courtNum}</div>
                        <div class="team flex gap-1.5 w-32 justify-center">
                            ${createPlayerHTML(court.team1[0], historyIndex, courtIndex, 0, 0)}
                            ${createPlayerHTML(court.team1[1], historyIndex, courtIndex, 0, 1)}
                        </div>
                        <span class="text-slate-300 font-bold text-sm">VS</span>
                        <div class="team flex gap-1.5 w-32 justify-center">
                            ${createPlayerHTML(court.team2[0], historyIndex, courtIndex, 1, 0)}
                            ${createPlayerHTML(court.team2[1], historyIndex, courtIndex, 1, 1)}
                        </div>
                    </div>
                `).join('');

                let restingHTML = `
                    <div class="mt-2 pt-2 border-t border-slate-100 text-sm flex items-baseline gap-2">
                        <strong class="text-slate-600 font-medium">พัก:</strong>
                        <span class="text-slate-500 flex-wrap">${match.resting.map(p => p.name).join(', ') || 'ไม่มี'}</span>
                    </div>
                `;

                historyCard.innerHTML = `
                    <h3 class="text-base font-semibold text-slate-600 mb-2">รอบที่ ${match.round}</h3>
                    ${courtsHTML}
                    ${restingHTML}
                `;
                historyContainer.appendChild(historyCard);
            });
        }

        createPlayerHTML = function(player, historyIndex, courtIndex, teamIndex, playerIndex) {
            if (!player) return `<div class="p-2 w-24 h-8"></div>`;
            const isHistory = historyIndex !== null;
            const levelColors = {
                'C': 'bg-sky-500 text-white',
                'B': 'bg-green-500 text-white',
                'A': 'bg-orange-500 text-white'
            };
            return `
                <div class="player-slot bg-slate-100 p-1.5 rounded-md w-24 text-center cursor-pointer hover:bg-slate-200 transition text-xs flex items-center gap-1.5" 
                     ${isHistory ? `data-history-index="${historyIndex}"` : ''}
                     data-court-index="${courtIndex}" 
                     data-team-index="${teamIndex}" 
                     data-player-index="${playerIndex}"
                     data-player-id="${player.id}">
                     <span class="level-indicator ${levelColors[player.level]} font-bold w-4 h-4 flex items-center justify-center rounded-full text-xs">${player.level}</span>
                    <span class="font-medium flex-1 text-left">${player.name}</span>
                </div>
            `;
        }

        renderPlayerList = function() {
            playerListContainer.innerHTML = '';
            playerTotalCount.textContent = players.length;

            if (players.length === 0) {
                playerListContainer.innerHTML = `<p class="text-slate-400 text-sm">ยังไม่มีผู้เล่น...</p>`;
                return;
            }

            const sortedPlayers = [...players].sort((a, b) => a.gamesPlayed - b.gamesPlayed || a.name.localeCompare(b.name, 'th'));
            
            const levelColors = {
                'C': 'bg-sky-500 text-white',
                'B': 'bg-green-500 text-white',
                'A': 'bg-orange-500 text-white'
            };

            sortedPlayers.forEach(player => {
                const playerTag = document.createElement('div');
                playerTag.className = 'player-tag bg-slate-100 inline-flex items-center gap-2 py-1 pl-2.5 pr-1 rounded-full border border-slate-200';
                
                playerTag.innerHTML = `
                    <button data-player-id="${player.id}" class="level-btn ${levelColors[player.level]} text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full cursor-pointer transition-colors">${player.level}</button>
                    <span class="font-medium text-sm">${player.name}</span>
                    <span class="text-xs bg-slate-200 text-slate-600 font-semibold w-10 text-center py-0.5 rounded-full">${player.gamesPlayed}</span>
                    <button data-player-id="${player.id}" data-player-name="${player.name}" class="remove-player-btn text-slate-400 hover:text-red-500 font-bold text-base leading-none w-5 h-5 flex items-center justify-center rounded-full hover:bg-red-100 transition-colors">&times;</button>
                `;
                playerListContainer.appendChild(playerTag);
            });
        }
        
        renderPartnershipStats = function() {
            statsList.innerHTML = '';
            const stats = [];

            for (const key in partnershipHistory) {
                const count = partnershipHistory[key];
                if (count > 0) {
                    const [p1Id, p2Id] = key.split('-');
                    const player1 = players.find(p => p.id == p1Id);
                    const player2 = players.find(p => p.id == p2Id);

                    if (player1 && player2) {
                        stats.push({
                            text: `${player1.name} & ${player2.name}: ${count} ครั้ง`,
                            count: count
                        });
                    }
                }
            }

            if (stats.length === 0) {
                statsList.innerHTML = '<p>ยังไม่มีข้อมูลการจับคู่</p>';
                return;
            }

            stats.sort((a, b) => b.count - a.count);

            stats.forEach(stat => {
                const p = document.createElement('p');
                p.textContent = stat.text;
                statsList.appendChild(p);
            });
        }
        
        // --- Modal Management ---
        openPlayerModal = function(context) {
            editingContext = context;
            modalPlayerList.innerHTML = '';
            
            const isHistory = context.historyIndex !== null;
            const match = isHistory ? history[context.historyIndex] : currentMatch;
            
            const playersInMatch = match.courts.flatMap(c => [...c.team1, ...c.team2]).filter(Boolean).map(p => p.id);
            const availableForSwap = players.filter(p => !playersInMatch.includes(p.id));
            
            const levelColors = {
                'C': 'bg-sky-500 text-white',
                'B': 'bg-green-500 text-white',
                'A': 'bg-orange-500 text-white'
            };

            if (availableForSwap.length === 0) {
                 modalPlayerList.innerHTML = `<p class="text-slate-500 col-span-2 text-center text-sm">ไม่มีผู้เล่นที่พักให้สลับ</p>`;
            } else {
                availableForSwap.forEach(player => {
                    const playerBtn = document.createElement('button');
                    playerBtn.className = 'w-full text-left p-2 rounded-lg hover:bg-sky-100 transition text-sm flex items-center gap-2';
                    playerBtn.innerHTML = `
                        <span class="${levelColors[player.level]} font-bold w-5 h-5 flex items-center justify-center rounded-full text-xs">${player.level}</span>
                        <span class="flex-1">${player.name}</span>
                        <span class="text-xs bg-slate-200 text-slate-600 font-semibold px-2 py-0.5 rounded-full">${player.gamesPlayed} เกม</span>
                        ${player.consecutiveRests > 1 ? `<span class="bg-amber-200 text-amber-800 text-xs font-bold w-4 h-4 flex items-center justify-center rounded-full">${player.consecutiveRests}</span>` : ''}
                    `;
                    playerBtn.onclick = () => selectPlayerForSwap(player);
                    modalPlayerList.appendChild(playerBtn);
                });
            }
            playerModal.classList.add('is-open');
        }

        closePlayerModal = function() {
            playerModal.classList.remove('is-open');
            editingContext = null;
        }

        selectPlayerForSwap = function(newPlayer) {
            if (!editingContext) return;

            const { historyIndex, courtIndex, teamIndex, playerIndex } = editingContext;
            const isHistory = historyIndex !== null;
            const matchToEdit = isHistory ? history[historyIndex] : currentMatch;
            
            const teamKey = teamIndex === 0 ? 'team1' : 'team2';
            const oldPlayer = matchToEdit.courts[courtIndex][teamKey][playerIndex];

            if (oldPlayer.id === newPlayer.id) {
                closePlayerModal();
                return;
            }

            const restingIndex = matchToEdit.resting.findIndex(p => p.id === newPlayer.id);

            if (restingIndex !== -1) {
                const playerFromRest = matchToEdit.resting[restingIndex];
                
                matchToEdit.courts[courtIndex][teamKey][playerIndex] = playerFromRest;
                matchToEdit.resting.splice(restingIndex, 1, oldPlayer);

                const pOld = players.find(p => p.id === oldPlayer.id);
                const pNew = players.find(p => p.id === newPlayer.id);
                if(pOld) {
                    pOld.gamesPlayed--;
                    pOld.consecutiveRests = 1;
                }
                if(pNew) {
                    pNew.gamesPlayed++;
                    pNew.consecutiveRests = 0;
                }

            } else {
                 console.error("Swap failed: Selected player not found in the resting list for that round.");
            }
            
            renderAll();
            saveState();
            closePlayerModal();
        }

        resetApp = function() {
            round = 0;
            history = [];
            currentMatch = { round: 0, courts: [], resting: [] };
            players.forEach(p => { 
                p.gamesPlayed = 0;
                p.consecutiveRests = 0;
            });
            partnershipHistory = {};
            
            resultsSection.style.display = 'none';
            historySection.style.display = 'none';
            messageArea.textContent = '';

            restingPlayersContainer.className = 'mt-6 bg-white p-4 rounded-2xl shadow-sm transition-colors duration-300';
            restingPlayersContainer.querySelector('h3').className = 'text-lg font-semibold text-slate-600 mb-3';
            
            renderPlayerList();
            saveState();
        }

        clearAll = function() {
            players = [];
            resetApp();
            localStorage.removeItem(STORAGE_KEY);
        }

        openConfirmModal = function(title, message, onConfirm) {
            confirmTitle.textContent = title;
            confirmMessage.textContent = message;
            pendingAction = onConfirm;
            confirmModal.classList.add('is-open');
        }

        closeConfirmModal = function() {
            confirmModal.classList.remove('is-open');
            pendingAction = null;
        }

        // --- Event Listeners ---
        addPlayersBtn.addEventListener('click', handleAddPlayers);
        generateRoundBtn.addEventListener('click', generateNewRound);
        reshuffleBtn.addEventListener('click', reshuffleCurrentRound);
        
        resetBtn.addEventListener('click', () => {
            openConfirmModal('ยืนยันการรีเซ็ตเกม', 'คุณแน่ใจหรือไม่ว่าต้องการรีเซ็ตเกม? จำนวนเกมของผู้เล่นทั้งหมดจะถูกตั้งเป็น 0', resetApp);
        });

        clearAllBtn.addEventListener('click', () => {
            openConfirmModal('ยืนยันการล้างข้อมูล', 'คุณแน่ใจหรือไม่ว่าต้องการล้างข้อมูลทั้งหมด? การกระทำนี้ไม่สามารถย้อนกลับได้', clearAll);
        });

        courtCountInput.addEventListener('change', saveState);
        
        playerListContainer.addEventListener('click', (e) => {
            const removeBtn = e.target.closest('.remove-player-btn');
            if (removeBtn) {
                const playerId = parseFloat(removeBtn.dataset.playerId);
                const playerName = removeBtn.dataset.playerName;
                openConfirmModal('ยืนยันการลบผู้เล่น', `คุณแน่ใจหรือไม่ว่าต้องการลบ "${playerName}"?`, () => {
                    removePlayer(playerId);
                });
                return;
            }

            const levelBtn = e.target.closest('.level-btn');
            if (levelBtn) {
                const playerId = parseFloat(levelBtn.dataset.playerId);
                changePlayerLevel(playerId);
            }
        });

        currentRoundContainer.addEventListener('click', (e) => {
            const reshuffleCourtBtn = e.target.closest('.reshuffle-court-btn');
            if (reshuffleCourtBtn) {
                const courtIndex = parseInt(reshuffleCourtBtn.dataset.courtIndex, 10);
                reshuffleSingleCourt(courtIndex);
                return;
            }
            
            const slot = e.target.closest('.player-slot');
            if (slot) {
                const context = {
                    historyIndex: null, // Only for current round
                    courtIndex: parseInt(slot.dataset.courtIndex, 10),
                    teamIndex: parseInt(slot.dataset.teamIndex, 10),
                    playerIndex: parseInt(slot.dataset.playerIndex, 10)
                };
                openPlayerModal(context);
            }
        });

        historyContainer.addEventListener('click', (e) => {
             const slot = e.target.closest('.player-slot');
            if (slot) {
                const context = {
                    historyIndex: parseInt(slot.dataset.historyIndex, 10),
                    courtIndex: parseInt(slot.dataset.courtIndex, 10),
                    teamIndex: parseInt(slot.dataset.teamIndex, 10),
                    playerIndex: parseInt(slot.dataset.playerIndex, 10)
                };
                openPlayerModal(context);
            }
        });

        toggleStatsBtn.addEventListener('click', () => {
            const isHidden = statsContainer.classList.toggle('hidden');
            if (!isHidden) {
                renderPartnershipStats();
                toggleStatsBtn.textContent = 'ซ่อนสถิติ';
            } else {
                toggleStatsBtn.textContent = 'ดูสถิติคู่';
            }
        });


        // Modal Listeners
        modalCloseBtn.addEventListener('click', closePlayerModal);
        playerModal.addEventListener('click', (e) => {
            if (e.target === playerModal) {
                closePlayerModal();
            }
        });

        confirmYesBtn.addEventListener('click', () => {
            if (typeof pendingAction === 'function') {
                pendingAction();
            }
            closeConfirmModal();
        });
        confirmNoBtn.addEventListener('click', closeConfirmModal);
        confirmModal.addEventListener('click', (e) => {
            if (e.target === confirmModal) {
                closeConfirmModal();
            }
        });

        // --- Initialization ---
        document.addEventListener('DOMContentLoaded', loadState);